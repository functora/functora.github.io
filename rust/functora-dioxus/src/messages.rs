...
impl I18N for Msg {
    fn render_eng(&self) -> String {
        match self {
            ...
            Self::EnvError(e) => return format!("Environment variable error: {e}"),
            Self::ChannelError(e) => return format!("Channel receive error: {e}"),
            Self::NotJsonObject(e) => return format!("Expected JSON object, got: {e}"),
            #[cfg(target_os = "android")]
            Self::JniError(e) => return format!("JNI error: {e}"),
    ...
    fn render_spa(&self) -> String {
        match self {
            ...
            Self::ErrorTitle(e) => return format!("Error: {e}"),
            Self::EnvError(e) => return format!("Error de variable de entorno: {e}"),
            Self::ChannelError(e) => return format!("Error de recepción en canal: {e}"),
            Self::NotJsonObject(e) => return format!("Se esperaba un objeto JSON, se obtuvo: {e}"),
            #[cfg(target_os = "android")]
            Self::JniError(e) => return format!("Error JNI: {e}"),
            Self::ErrorTitleLabel => "Error",
    ...
    fn render_rus(&self) -> String {
        match self {
            ...
            Self::EnvError(e) => return format!("Ошибка переменной окружения: {e}"),
            Self::ChannelError(e) => return format!("Ошибка получения из канала: {e}"),
            Self::NotJsonObject(e) => return format!("Ожидался JSON-объект, получено: {e}"),
            #[cfg(target_os = "android")]
            Self::JniError(e) => return format!("Ошибка JNI: {e}"),
