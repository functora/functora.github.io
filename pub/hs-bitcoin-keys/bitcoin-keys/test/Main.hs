...
tt =
  testGroup
    ...
    [ testCase "Show Pub" $ do
        show pub1c
          @?= "Pub 024ed59d3f6548c41172ec26b9144f547185cb2a9f28fccb6e3ae53dc4f1827154",
      testCase "Show Prv" $ do
        show prv1
          @?= "Prv 76deed24fcdfb479656af6c5c4ab906090c5ee39da1c1ad1f1ad56dfce378a03",
      testCase "Show Tweak" $ do
        show tweak1
          @?= "Tweak 0ef561fb5f4e9e2319a5b75ed3be3c895e6ec272c0b5185dc414fb7e47c3a770",
...
fromBase16 a =
  handler $ B16.decode a
  where
#if MIN_VERSION_base16_bytestring(1,0,0)
      handler :: Either String B.ByteString -> B.ByteString
      handler =
        either (const $ error ("Invalid base16 string: " <> show a)) id
#else
      handler :: (B.ByteString, B.ByteString) -> B.ByteString
      handler = \case
        (success, "") -> success
        failure -> error ("Invalid base16 string: " <> show a)
#endif
