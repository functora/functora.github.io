...
pub fn video_thumbnail(data: &[u8]) -> Option<Vec<u8>> {
    ...
    let (track_id, avcc, first, last) = {
        ...
        let first = track
            ...
            .unwrap_or(1);
        let last = first.saturating_add(MAX_SAMPLES - 1).min(track.sample_count());
        (*id, avcc, first, last)
    };
    let mut config = vec![
    ...
    for nal in avcc_config.sps_nals.iter().chain(avcc_config.pps_nals.iter()) {
...
fn yuv_to_rgb(frame: &rust_h264::decoder::Frame) -> Vec<u8> {
    ...
    for y in 0..height {
        for x in 0..width {
            ...
            let g = u8::try_from(((yy - 352 * uu - 731 * vv + 0x200) >> 10).clamp(0, 255)).unwrap_or(u8::MAX);
...
static MEMO: LazyLock<Mutex<HashMap<u64, Option<String>>>> = LazyLock::new(|| Mutex::new(HashMap::new()));
...
fn fit(width: u32, height: u32, max_w: u32, max_h: u32) -> (u32, u32) {
    ...
    if u64::from(max_w) * u64::from(height) <= u64::from(max_h) * u64::from(width) {
        ...
        let scaled_w = (u64::from(width) * u64::from(max_h) + u64::from(height) / 2) / u64::from(height);
        (u32::try_from(scaled_w.clamp(1, u64::from(max_w))).unwrap_or(1), max_h)
    }
}
