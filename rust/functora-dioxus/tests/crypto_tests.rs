...
fn chacha20_roundtrip() {
    ...
    let encrypted = encrypt_symmetric(plaintext, "pw", CipherType::ChaCha20Poly1305, AAD).expect("encrypt");
    let decrypted = decrypt_symmetric(&encrypted, "pw", AAD).expect("decrypt");
...
fn aes256gcm_roundtrip() {
    ...
    let encrypted = encrypt_symmetric(plaintext, "pw", CipherType::Aes256Gcm, AAD).expect("encrypt");
    let decrypted = decrypt_symmetric(&encrypted, "pw", AAD).expect("decrypt");
    assert_eq!(plaintext.to_vec(), decrypted);
}
...
fn wrong_password_fails() {
    fast_kdf();
    let encrypted = encrypt_symmetric(b"data", "correct", CipherType::Aes256Gcm, AAD).expect("encrypt");
...
fn wrong_aad_fails() {
    fast_kdf();
    let encrypted = encrypt_symmetric(b"data", "pw", CipherType::Aes256Gcm, AAD).expect("encrypt");
...
fn derive_key_is_consistent_and_salt_sensitive() {
    ...
    let k1a = derive_key("pw", &salt1, Kdf::Argon2id).expect("derive");
    let k1b = derive_key("pw", &salt1, Kdf::Argon2id).expect("derive");
    let k2 = derive_key("pw", &salt2, Kdf::Argon2id).expect("derive");
    assert_eq!(k1a, k1b);
    assert_ne!(k1a, k2);
...
fn empty_plaintext_roundtrip() {
    fast_kdf();
    let encrypted = encrypt_symmetric(b"", "pw", CipherType::ChaCha20Poly1305, AAD).expect("encrypt");
    assert_eq!(b"".to_vec(), decrypt_symmetric(&encrypted, "pw", AAD).expect("decrypt"));
}
...
fn stream_roundtrip_large_payload() {
    ...
    let encrypted = stream_encrypt_symmetric(&plaintext, "pw", CipherType::Aes256Gcm, AAD).expect("encrypt");
    assert_eq!(
        plaintext,
        stream_decrypt_symmetric(&encrypted, "pw", AAD).expect("decrypt")
    );
}
...
fn stream_tampered_ciphertext_fails() {
    fast_kdf();
    let encrypted =
        stream_encrypt_symmetric(b"payload data", "pw", CipherType::ChaCha20Poly1305, AAD).expect("encrypt");
...
fn stream_recover_rejects_bad_sizes() {
    fast_kdf();
    let encrypted = stream_encrypt_symmetric(b"data", "pw", CipherType::Aes256Gcm, AAD).expect("encrypt");
