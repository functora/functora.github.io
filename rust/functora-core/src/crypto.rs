...
fn crypto_fn(
    ...
    match (cipher, kind) {
        (CipherType::ChaCha20Poly1305, CipherKind::Encrypt) => {
        ...
        (CipherType::ChaCha20Poly1305, CipherKind::Decrypt) => {
            ChaCha20Poly1305::new_from_slice(key)?
                .decrypt(
                    chacha20poly1305::Nonce::from_slice(nonce),
                    Payload { msg: data, aad },
...
pub fn stream_decrypt_symmetric(
    data: &EncryptedNote,
    password: &str,
    aad: &[u8],
) -> Result<Vec<u8>, Error> {
...
pub fn derive_key(password: &str, salt: &[u8], kdf: Kdf) -> Result<Vec<u8>, Error> {
    ...
    match kdf {
        Kdf::Argon2id => {
            ...
            Argon2::new(Argon2Algo, Argon2V, params).hash_password_into(
                password.as_bytes(),
                salt,
                &mut key,
            )?;
