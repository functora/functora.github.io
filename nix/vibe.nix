{pkgs ? import <nixpkgs> {}}: let
  ...
  agentsMd = pkgs.writeTextFile {
    name = "AGENTS.md";
    text = ''
      Read the `README.md` file if available. Study the project source code. Write your code in a strictly functional style while ensuring high efficiency. Avoid mutable variables (`mut`), imperative constructs (e.g., `for`, `while`, `loop`, or `return`), and instead use functional iterators and control flow with `?` operator. Eliminate redundant closures and variables. Maximize method and function chaining. Avoid `.clone()`, unnecessary allocations, and extra dependencies. Prefer the standard library exclusively when possible. Follow the existing project code style. Use meaningful, context-clear identifiers with a preference for brevity, as in the original code. Do not include comments; the code must be self-explanatory. To check Rust code use the command `cargo clippy --all-features`. The code should not have any warnings or errors. To test Rust code use the command `cargo test --all-features`. To get Rust test coverage report use the command `cargo tarpaulin --all-features -o Lcov`, the report is written into `lcov.info` file. Test coverage should be as high as possible.
    '';
  };
