...
impl CryptonoteApp {
    pub(crate) fn screen_privacy(&mut self, ui: &mut egui::Ui) {
        let lang = self.lang();
        _ = ui.label(
            egui::RichText::new(BaseMsg::PrivacyPolicyTitle.render(lang))
                .size(20.0)
                .strong(),
        );
        let () = ui.add_space(8.0);
        _ = ScrollArea::vertical().show(ui, |inner| {
            _ = inner.label(BaseMsg::PrivacyText.render(lang));
        });
        let () = ui.add_space(8.0);
        if ui
            .add(
