  ...
  where
    ...
    parseWorksheet root sheetRels = do
      ...
      pivotTables <- forM (allByType ptType sheetRels) $ \rel -> do
        ...
        bs <-
          note (MissingFile ptPath) $ Zip.fromEntry <$> Zip.findEntryByPath ptPath ar
...
extractSheet ar sst contentTypes caches wf = do
  ...
  file <-
    note (MissingFile filePath) $ Zip.fromEntry <$> Zip.findEntryByPath filePath ar
  cur <-
    fmap fromDocument . left (\ex -> InvalidFile filePath (T.pack $ show ex)) $
      parseLBS def file
  sheetRels <- getRels ar filePath
  ...
  let commentsType =
      ...
      legacyDrPath =
        fmap relTarget . flip Relationships.lookup sheetRels
          =<< listToMaybe legacyDrRId

  commentsMap :: Maybe CommentTable <-
    maybe (Right Nothing) (getComments ar legacyDrPath) commentTarget
  ...
  let pageSetup = listToMaybe $ cur $/ element (n_ "pageSetup") >=> fromCursor
      ...
      parseRow c = do
        ...
        let prop =
              RowProps
                { rowHeight = do
                    h <- listToMaybe $ fromAttribute "ht" c
      ...
      condFormtattings =
        M.fromList . map unCfPair $
          cur $/ element (n_ "conditionalFormatting") >=> fromCursor
      ...
      tableIds =
        cur
          $/ element (n_ "tableParts")
          &/ element (n_ "tablePart")
...
xmlCursorRequired ar fname = do
  ...
  cur <-
    left (\ex -> InvalidFile fname (T.pack $ show ex)) $
...
getComments ::
  Zip.Archive -> Maybe FilePath -> FilePath -> Parser (Maybe CommentTable)
getComments ar drp fp = do
  mCurComments <- xmlCursorOptional ar fp
  mCurDr <- maybe (return Nothing) (xmlCursorOptional ar) drp
  return
    (liftA2 hide (hidden <$> mCurDr) . listToMaybe . fromCursor =<< mCurComments)
  where
    ...
    hidden cur =
      cur
        $/ checkElement visibleShape
        &/ element (x "ClientData")
        >=> shapeCellRef
    visibleShape Element {..} =
      elementName == (v "shape")
        && maybe
          False
          (any ("visibility:hidden" ==) . T.split (== ';'))
          (M.lookup "style" elementAttributes)
...
readWorkbook ::
  Zip.Archive -> Parser ([WorksheetFile], DefinedNames, Caches, DateBase)
readWorkbook ar = do
  ...
  let mkDefinedName :: Cursor -> [(Text, Maybe Text, Text)]
      ...
      names =
        cur
          $/ element (n_ "definedNames")
          &/ element (n_ "definedName")
          >=> mkDefinedName
  sheets <-
    sequence $
      cur
        $/ element (n_ "sheets")
        &/ element (n_ "sheet")
        >=> liftA3 (worksheetFile wbPath wbRels)
          <$> attribute "name"
          <*> fromAttributeDef "state" def
          <*> fromAttribute (odr "id")
  let cacheRefs =
        cur
          $/ element (n_ "pivotCaches")
          &/ element (n_ "pivotCache")
  ...
  let dateBase =
        bool DateBase1900 DateBase1904 . fromMaybe False . listToMaybe $
...
worksheetFile ::
  FilePath -> Relationships -> Text -> SheetState -> RefId -> Parser WorksheetFile
...
getRels ar fp = do
  ...
  return $
    maybe
      Relationships.empty
      (setTargetsFrom fp . headNote "Missing rels" . fromCursor)
      c
...
lookupRelPath fp rels rId =
  logicalNameToZipItemName . relTarget
    <$> note (InvalidRef fp rId) (Relationships.lookup rId rels)
