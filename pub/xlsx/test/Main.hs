    ...
    testGroup
      ...
      [ testCase "write . read == id" $ do
        ...
        testCase "fromRight . parseStyleSheet . renderStyleSheet == id" $
          testStyleSheet
            @==? fromRight (parseStyleSheet (renderStyleSheet testStyleSheet)),
        ...
        testCase "correct shared strings parsing: single underline" $
          [withSingleUnderline testSharedStringTable]
            @=? parseBS testStringsWithSingleUnderline,
        ...
        testCase "formatted . toFormattedCells = id" $ do
          ...
          testFormattedCells
            @==? toFormattedCells
              (formattedCellMap fmtd)
              (formattedMerges fmtd)
          ...
          [ testGroup
              ...
              [ testProperty
                ...
                testCase "should otherwise infer visible state by default" $
                  Right (Just Visible)
                    @=? (fmap sheetStateOfDefXlsx . toXlsxEitherFast) (fromXlsx testTime defXlsx)
              ],
            testGroup
              "toXlsxEither"
              [ testProperty "pure state == toXlsxEither (fromXlsx (defXlsxWithState state))" $
                ...
                testCase "should otherwise infer visible state by default" $
                  Right (Just Visible)
                    @=? (fmap sheetStateOfDefXlsx . toXlsxEither) (fromXlsx testTime defXlsx)
              ]
          ],
        CommonTests.tests,
        CondFmtTests.tests,
