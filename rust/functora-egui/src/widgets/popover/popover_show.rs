    ...
    pub fn show(
        ...
        let _ = popup.show(|popup_ui: &mut egui::Ui| {
            ...
            let w = if spacing.is_mobile() {
                crate::utils::clamp_overlay_width(
                    screen_w,
                    200.0,
            ...
            popup_ui.set_min_width(w.min(screen_w - 2.0 * spacing.page_padding));
            popup_ui.set_max_width((screen_w - 2.0 * spacing.page_padding).max(0.0));
            content(popup_ui);
        });
    }
}
