  ...
  where
    ...
    fCols =
      M.fromList
        [ ( 1,
          ...
          ( 2,
            CustomFiltersAnd
              (CustomFilter FltrGreaterThanOrEqual "0")
    ...
    rowProps =
      M.fromList
        [ ( 1,
            RowProps
              { rowHeight = Just (CustomHeight 50),
                rowStyle = Just 3,
                rowHidden = False
              }
          )
...
testCondFormattedResult = CondFormatted styleSheet formattings
  where
    ...
    formattings =
      M.fromList
        [ (SqRef [CellRef "A1:A2", CellRef "B2:B3"], [cfRule1, cfRule2]),
