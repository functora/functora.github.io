...
use egui_shadcn::{
    Badge, BadgeVariant, Button, ButtonVariant, Card, ComponentSize, Flex, LucideIcon, Separator,
    Typography, TypographyVariant,
};

impl crate::app::ShowcaseApp {
    pub(crate) fn demo_overview(ui: &mut egui::Ui) {
        _ = Typography::lead(
        ...
        _ = Card::new().show(ui, |ui70| {
            ...
            ui70.add_space(4.0);
            _ = Typography::new(
                "This page is an interactive showcase of every egui-shadcn layout, component, \
        ...
        for (cat_name, cat_icon, items) in CATEGORIES {
            ...
            _ = Flex::row().gap(4.0).wrap().show(ui, |f| {
                for def in *items {
                    _ = f.add(Badge::new(def.name).variant(BadgeVariant::Secondary));
                }
            });
            ui.add_space(8.0);
        }
