use cryptonote::crypto::{
    decrypt_symmetric, encrypt_symmetric, CipherType,
};
use cryptonote::encoding::{
    build_url, decode_note, encode_note,
    extract_note_param, generate_qr_code, NoteData,
};
...
fn test_encode_decode_plaintext_unicode() {
    let note = NoteData::PlainText(
        "Привет мир! ¡Hola! こんにちは".to_string(),
    );
    let encoded =
        encode_note(&note).expect("Encoding failed");
    let decoded =
...
fn test_build_url_roundtrip() {
    let note =
        NoteData::PlainText("Round trip test".to_string());
    let url =
        build_url("https://example.com", &note).unwrap();
    let extracted = extract_note_param(&url)
        .expect("Extraction failed");
    let decoded =
        decode_note(&extracted).expect("Decoding failed");
    match decoded {
        NoteData::PlainText(text) => {
...
fn test_extract_note_param_basic() {
    let url =
        "https://example.com/?screen=view&note=abc123";
    let result =
        extract_note_param(url).expect("Extraction failed");
...
fn test_build_url_and_extract_roundtrip() {
    ...
    match decoded {
        NoteData::PlainText(text) => {
            assert_eq!(text, "Test with special chars: +/=")
        }
        _ => panic!("Expected PlainText"),
    }
}
