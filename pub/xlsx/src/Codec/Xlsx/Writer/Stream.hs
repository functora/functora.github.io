...
boilerplate settings sharedStrings' =
  [ ( zipEntry "xl/sharedStrings.xml",
      ZipDataSource $ writeSst sharedStrings' .| eventsToBS
    ),
...
doc ::
  (Monad m) => Name -> forall i. ConduitT i Event m () -> ConduitT i Event m ()
doc root docM = do
...
relationship ::
  (Monad m) => Text -> Int -> Text -> forall i. ConduitT i Event m ()
...
writeSst sharedStrings' =
  doc (n_ "sst")
    $ void
    $ traverse
      ( el (n_ "si") . el (n_ "t") . content . fst
      )
    $ sortBy (\(_, i) (_, y :: Int) -> compare i y)
    $ Map.toList sharedStrings'
...
writeWorkSheet ::
  (MonadReader SheetWriteSettings m) => Map Text Int -> ConduitT Row Event m ()
  ...
  mRowProp <-
    preview $
      #wsRowProperties
        ...
        . rowHeightLens
        . _Just
        . failing _CustomHeight _AutomaticHeight
