...
genAddr lnd =
  fmap NA.address
    $ liftLndResult
    =<< newAddress lnd (NA.NewAddressRequest NA.WITNESS_PUBKEY_HASH Nothing)

findUtxosByTxId ::
  (KatipContext m, MonadUnliftIO m) => LndEnv -> ByteString -> m [LU.Utxo]
findUtxosByTxId lnd txid' = do
  utxos <-
    LU.utxos <$> (liftLndResult =<< listUnspent lnd (LU.ListUnspentRequest 0 10 ""))
...
fundPsbtToAddr fAddr amt = do
  ...
  let bankAmt = amt * 2
  sendTrx <-
    liftLndResult =<< sendCoins lndBob (SC.SendCoinsRequest addrAlice bankAmt False)
  mine 2 Bob
...
signPsbt ::
  (LndTest m Owner) => FP.FundPsbtResponse -> m FNP.FinalizePsbtResponse
signPsbt psbt' = do
  lndAlice <- getLndEnv Alice
  liftLndResult
    =<< finalizePsbt lndAlice (FNP.FinalizePsbtRequest (FP.fundedPsbt psbt') "")
...
openChannelPsbt lndEnv toPubKey locFundAmt = do
  ...
  void
    $ spawnLink
    $ liftLndResult
    =<< openChannel (void . T.atomically . T.writeTChan chan) lndEnv openChannelRequest
  fundStep pcid chan
  where
    fundStep pcid chan = do
      ...
      case upd of
        OpenStatusUpdate
          _
          (Just (OpenStatusUpdatePsbtFund (ReadyForPsbtFunding faddr famt _))) -> do
            $logTM DebugS
              . logStr
            ...
            $logTM DebugS
              . logStr
              $ "Used psbt for funding:"
              <> inspect @Text sPsbtResp
            void
              $ liftLndResult
              =<< fundingStateStep lndEnv (psbtFinalizeReq pcid (Psbt $ FNP.signedPsbt sPsbtResp))
            fundStep pcid chan
        OpenStatusUpdate _ (Just (OpenStatusUpdateChanPending p)) -> do
          $logTM DebugS
            . logStr
            $ "Chan is pending... mining..."
            <> inspect @Text p
          mine 3 Bob
          fundStep pcid chan
...
spec = do
  it "fundPsbt"
    ...
    $ do
      ...
      sendTrx <-
        liftLndResult =<< sendCoins lndBob (SC.SendCoinsRequest addrAlice amt False)
