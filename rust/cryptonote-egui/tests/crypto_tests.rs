...
fn test_symmetric_wrong_password() {
    ...
    let encrypted = encrypt_symmetric(plaintext, "correct", CipherType::ChaCha20Poly1305)
        .expect("Encryption failed");
...
fn test_different_nonces_for_same_input() {
    ...
    let password = "password";
    let encrypted1 = encrypt_symmetric(plaintext, password, CipherType::ChaCha20Poly1305)
        .expect("Encryption failed");
    let encrypted2 = encrypt_symmetric(plaintext, password, CipherType::ChaCha20Poly1305)
        .expect("Encryption failed");
...
fn test_decrypt_tampered_ciphertext_fails() {
    ...
    let mut encrypted = encrypt_symmetric(plaintext, "pw", CipherType::ChaCha20Poly1305)
        .expect("Encryption failed");
...
fn test_encrypt_empty_password() {
    ...
    let encrypted =
        encrypt_symmetric(plaintext, "", CipherType::ChaCha20Poly1305).expect("Encryption failed");
...
fn test_new_notes_use_argon2id() {
    common::fast_kdf();
    let encrypted =
        encrypt_symmetric(b"data", "pw", CipherType::Aes256Gcm).expect("Encryption failed");
...
fn test_metadata_contains_no_password_or_key() {
    common::fast_kdf();
    let encrypted = encrypt_symmetric(b"data", "super-secret-password", CipherType::Aes256Gcm)
        .expect("Encryption failed");
...
fn stream_roundtrip(size: usize, cipher: CipherType) {
    ...
    let encrypted =
        stream_encrypt_symmetric(&plaintext, "pw", cipher).expect("Stream encryption failed");
    assert_eq!(
...
fn test_stream_wrong_password() {
    ...
    let encrypted = stream_encrypt_symmetric(&plaintext, "pw", CipherType::Aes256Gcm)
        .expect("Encryption failed");
...
fn test_stream_reordered_chunks_fail() {
    common::fast_kdf();
    let plaintext: Vec<u8> = (0..STREAM_CHUNK * 2)
        .map(|i| u8::try_from(i % 256).unwrap())
        .collect();
    let encrypted = stream_encrypt_symmetric(&plaintext, "pw", CipherType::ChaCha20Poly1305)
        .expect("Encryption failed");
