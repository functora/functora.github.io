...
import Codec.Xlsx.Types.Internal.Relationships as Relationships hiding
  ( lookup,
  )
...
fromXlsx pt xlsx =
  ...
  where
    ...
    entries =
      Zip.toEntry "[Content_Types].xml" t (contentTypesXml files)
...
singleSheetFiles n cells pivFileDatas ws tblIdRef = do
  ...
  let sheetFilePath = "xl/worksheets/sheet" <> show n <> ".xml"
      ...
      sheetRels =
        if null referencedFiles
          then []
          else
            [ FileData
                ("xl/worksheets/_rels/sheet" <> show n <> ".xml.rels")
                "application/vnd.openxmlformats-package.relationships+xml"
                "relationships"
                sheetRelsXml
            ]
    ...
    rowEl (r, cells) = do
      let mProps = M.lookup r rh
          hasHeight = case rowHeight =<< mProps of
            ...
            _ -> False
          ht = do
            Just height <- [rowHeight =<< mProps]
            let h = case height of
                  CustomHeight x -> x
                  AutomaticHeight x -> x
            return ("ht", txtd h)
          s = do
            Just st <- [rowStyle =<< mProps]
            return ("s", txti st)
...
genComments ::
  Int -> Cells -> STRef s Int -> ST s (Maybe (Element, [ReferencedFileData]))
genComments n cells ref =
  ...
  where
    comments =
      concatMap (\(row, rowCells) -> mapMaybe (maybeCellComment row) rowCells) cells
...
genDrawing ::
  Int -> STRef s Int -> Drawing -> ST s (Element, ReferencedFileData, [FileData])
...
coreXml created creator =
  ...
  where
    ...
    root =
      Element
        (namespaced "cp" "coreProperties")
        M.empty
        [ nEl
            (namespaced "dcterms" "created")
            (M.fromList [(namespaced "xsi" "type", "dcterms:W3CDTF")])
            [NodeContent date],
...
appXml sheetNames =
  ...
  where
    ...
    extPropNm n =
      nm "http://schemas.openxmlformats.org/officeDocument/2006/extended-properties" n
    extPropEl n = nEl (extPropNm n) M.empty
    vTypeEl0 n = vTypeEl n M.empty
    vTypeEl =
      nEl . nm "http://schemas.openxmlformats.org/officeDocument/2006/docPropsVTypes"
...
bookFiles xlsx = runST $ do
  ...
  let sheets = xlsx ^. xlSheets & mapped %~ snd
      ...
      sharedStrings =
        ( ssRId,
          FileData "xl/sharedStrings.xml" (smlCT "sharedStrings") "sharedStrings" $
            ssXml shared
        )
  stRId <- nextRefId ref
  let style =
    ...
    root =
      addNS "http://schemas.openxmlformats.org/spreadsheetml/2006/main" Nothing $
        elementListSimple
          ...
          ( [ leafElement
                "workbookPr"
                (catMaybes ["date1904" .=? justTrue (dateBase == DateBase1904)]),
...
contentTypesXml fds = renderLBS def $ Document (Prologue [] Nothing []) root []
  where
    root =
      addNS "http://schemas.openxmlformats.org/package/2006/content-types" Nothing $
          ...
          map
            ( \fd ->
                nEl
                  "Override"
                  ( M.fromList
                      [ ("PartName", T.concat ["/", T.pack $ fdPath fd]),
                        ("ContentType", fdContentType fd)
                      ]
                  )
                  []
            )
