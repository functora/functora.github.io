...
readCondition t _ =
  error $ "Unexpected conditional formatting type " ++ show t
...
instance FromXenoNode CfRule where
  fromXenoNode root = parseAttributes root $ do
    ...
    where
      ...
      readConditionX x =
        toAttrParser . Left $
          "Unexpected conditional formatting type " <> T.pack (show x)
...
instance FromCursor DataBarOptions where
  fromCursor cur = do
    ...
    case cfvos of
      ...
      ns -> do
        fail $
          "expected minimum and maximum cfvo nodes but see instead "
            ++ show (length ns)
            ++ " cfvo nodes"
...
conditionData (AboveAverage i sDevs) =
  ( "aboveAverage",
    M.fromList $
      ["aboveAverage" .= True]
        ++ catMaybes
          [ "equalAverage" .=? justNonDef Exclusive i,
            "stdDev" .=? sDevs
          ],
    []
  )
...
conditionData (BottomNPercent n) =
  ("top10", M.fromList ["bottom" .= True, "rank" .= n, "percent" .= True], [])
...
conditionData (CellIs opExpr) = ("cellIs", M.fromList ["operator" .= op], formulas)
  where
    (op, formulas) = operatorExpressionData opExpr
