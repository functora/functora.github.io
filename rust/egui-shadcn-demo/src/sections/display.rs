...
impl crate::app::ShowcaseApp {
    ...
    pub(crate) fn demo_label(&mut self, ui: &mut egui::Ui) {
        ...
        _ = Flex::column().gap(8.0).show(ui, |f| {
            ...
            _ = f.add(egui_shadcn::Input::new(&mut self.flex_email).placeholder("you@example.com"));
            _ = f.ui(|ui52| {
    ...
    pub(crate) fn demo_icons(&mut self, ui: &mut egui::Ui) {
        ...
        ui.add_space(12.0);
        _ = egui_shadcn::Input::new(&mut self.icon_search)
            .placeholder("Search icons...")
            .desired_width(260.0)
            .show(ui);
        ui.add_space(8.0);
        let needle = self.icon_search.trim().to_ascii_lowercase();
        let icons: Vec<LucideIcon> = egui_shadcn::icons::lucide_icon::ALL
            .iter()
            .copied()
